...
//  flag 0 symmetric对称 1 periodic周期 针对fft length extend 1 sample
...
static float *__window_createHann(int length,int flag){
	...
	if(!(length&1)){ // even
		halfLen=length/2+flag; // periodic +1
	}
	else{ // odd
		halfLen=(length+1)/2;
	}
...
static float __besselZeroOne(float a){
	...
	for(int k=1;k<16;k++){ // 15~25, 8 is ok
		num=num*b;
		den=den*k;
		mid=num/den;
		sum=sum+mid*mid;
